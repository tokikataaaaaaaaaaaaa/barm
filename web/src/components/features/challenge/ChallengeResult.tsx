...
import { ChallengeResultData, getResultType } from '@/lib/types'
          ...
          {result.rankings.slice(0, 4).map((ranking, index) => (
            <div
              ...
              className={`flex items-center gap-3 p-2 rounded-lg ${
                ranking.userId === currentUserId ? 'bg-accent/10' : ''
              }`}
            >
              <span className="text-lg w-6 text-center">
                {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${index + 1}.`}
              </span>
              <Avatar fallback={ranking.displayName?.charAt(0) || '?'} size="xs" />
              <span className="flex-1 text-body-sm">{ranking.displayName || 'ユーザー'}</span>
